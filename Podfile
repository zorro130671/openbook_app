...
post_install do |installer|
  installer.pods_project.targets.each do |target|
    ...
    target.build_configurations.each do |config|
      ...
      # Enforce C++17 standard to fix gRPC-Core template issues with Xcode 16+
      config.build_settings['CLANG_CXX_LANGUAGE_STANDARD'] = 'c++17'
      config.build_settings['CLANG_CXX_LIBRARY'] = 'libc++'
      config.build_settings['OTHER_CPLUSPLUSFLAGS'] = '-std=c++17'
      # Optional: Exclude arm64 for simulator if you encounter architecture issues
      config.build_settings['EXCLUDED_ARCHS[sdk=iphonesimulator*]'] = 'arm64'
    end

    # Fix for BoringSSL-GRPC -G flag and template issues
